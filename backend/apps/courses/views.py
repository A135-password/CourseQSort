...
from apps.courses.models import Major, Teacher, Classroom, Course, Student, CourseAssignment, ClassGroup
from apps.courses.serializers import (
    MajorSerializer, TeacherSerializer, ClassroomSerializer,
    CourseListSerializer, CourseDetailSerializer,
    CourseCreateSerializer, StudentSerializer,
    CourseAssignmentSerializer, ClassGroupSerializer,
)
...
class CourseViewSet(viewsets.ModelViewSet):
    ...
    def batch_delete(self, request):
        ...
        if not ids:
            return Response({'detail': '请选择要删除的课程'},
                            status=status.HTTP_400_BAD_REQUEST)
    ...
    def delete_all(self, request):
        ...
        if not password:
            return Response({'detail': '请输入教务密码'},
                            status=status.HTTP_400_BAD_REQUEST)
...
class ClassroomViewSet(viewsets.ModelViewSet):
    ...
    def import_json(self, request):
        ...
        for r in records:
            ...
            obj, is_new = Classroom.objects.get_or_create(
                name=name, building=building, defaults=defaults)
            if is_new:
                created += 1
        return Response({'imported': created, 'total': len(records)})
...
class MajorViewSet(viewsets.ModelViewSet):
    ...
    def import_json(self, request):
        ...
        for r in records:
            ...
            if not name:
                continue
            obj, is_new = Major.objects.get_or_create(name=name, defaults={
                'code': r.get('code', ''),
    ...
    def import_json(self, request):
        ...
        for r in records:
            ...
            obj, is_new = Student.objects.get_or_create(
                student_no=student_no, defaults=defaults)
            if is_new:
                created += 1
            else:
                # 更新外键
