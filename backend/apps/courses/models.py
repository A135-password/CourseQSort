...
class ClassGroup(models.Model):
    ...
    major = models.ForeignKey(
        Major, on_delete=models.CASCADE, related_name='classes',
        verbose_name='所属专业'
    )
...
class CourseScheduleItem(models.Model):
    course = models.ForeignKey(
        Course, on_delete=models.CASCADE, related_name='schedule_items'
    )
...
class CourseAssignment(models.Model):
    ...
    grade = models.CharField(
        max_length=20, blank=True, default='',
        help_text='目标年级，如 2024（为空表示不限年级）'
    )
    class_identification = models.CharField(
        max_length=200, blank=True, default='',
        help_text='目标班级，如 计科2401（为空表示不限班级）'
    )
    created_at = models.DateTimeField(auto_now_add=True)
