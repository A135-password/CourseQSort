...
from apps.student.serializers import (
    ScheduleSerializer, ScheduleCourseSerializer,
    AvailableCourseSerializer, ConflictCourseSerializer,
    ConflictDetailSerializer, SelectCourseSerializer,
    FreeSlotSerializer, RecommendCourseSerializer,
)
...
class CourseListView(APIView):
    ...
    def get(self, request):
        ...
        courses = Course.objects.all().prefetch_related(
            'schedule_items__classroom', 'teachers'
        )
        if major_id:
        ...
        for c in courses:
            time_slots_raw = list(set(
                (item.day_of_week, item.period) for item in c.schedule_items.all()
            ))
                ...
                for e in Enrollment.objects.filter(user=request.user).select_related('course'):
                    ...
                    if overlap:
                        conflict_with.append({
                            ...
                            'name': e.course.name,
                            'time_slots': [
                                {'day_of_week': d, 'period': p} for d, p in overlap
                            ],
                        })
...
class ConflictDetailView(APIView):
    ...
    def get(self, request, pk=None):
        ...
        course_slots_raw = list(set(
            (item.day_of_week, item.period) for item in course.schedule_items.all()
        ))
        ...
        for e in Enrollment.objects.filter(user=request.user).select_related('course'):
            ...
            if overlap:
                for d, p in overlap:
                    ...
                    first_item = e.course.schedule_items.filter(
        ...
        return Response({
            ...
            'course_time_slots': [
                {'day_of_week': d, 'period': p} for d, p in course_slots_raw
            ],
...
class FreeSlotsView(APIView):
    ...
    def get(self, request):
        ...
        period_labels = {
            1: '第一二节', 2: '第三四节', 3: '第五六节',
            4: '第七八节', 5: '第九十节', 6: '第十一十二节',
            7: '第十三十四节', 8: '第十五十六节',
            9: '第十七十八节', 10: '第十九二十节', 11: '第二十一二十二节',
        }
...
class RecommendView(APIView):
    ...
    def get(self, request, day=None, period=None):
        try:
            ...
            return Response({'detail': 'Invalid day or period'},
