...
class StudentCourseApiTests(StudentTestMixin, TestCase):
    ...
    def test_select_course_rejects_conflicting_schedule(self):
        response = self.client.post(
            f'/api/v1/student/courses/{self.conflicting_course.id}/select/'
        )

        self.assertEqual(response.status_code, 409)
        self.assertEqual(response.data['status'], 'CONFLICT')
        self.assertFalse(
            Enrollment.objects.filter(user=self.user, course=self.conflicting_course).exists()
        )

    def test_select_course_rejects_full_course(self):
        response = self.client.post(
            f'/api/v1/student/courses/{self.full_course.id}/select/'
        )
    ...
    def test_select_course_creates_enrollment_when_slot_is_available(self):
        response = self.client.post(
            f'/api/v1/student/courses/{self.available_course.id}/select/'
        )

        self.assertEqual(response.status_code, 201)
        self.assertEqual(response.data['status'], 'SELECTED')
        self.assertTrue(
            Enrollment.objects.filter(user=self.user, course=self.available_course).exists()
        )
